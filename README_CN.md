...
- **不需要新的 API key**：直接使用你已经在终端里运行的 CLI AI，包括 Claude Code、Codex、本地 LLM CLI 或自定义终端 agent。
- **任意 CLI AI 接进 F12**：bridge 对 DevTools 面板走 WebSocket，对 agent 走 stdin/stdout，不绑定单一托管模型厂商。
- **DevTools 原生证据链**：页面文本、DOM、console、network、selector 和本地前端上下文都集中在同一个聊天面板里。
- **不同于 MCP-only 流程**：MCP 很适合作为工具后端；cc-devtools 把操作者界面直接放进 Chrome F12。
- **不同于 Playwright MCP**：Playwright 更适合脚本化自动化；cc-devtools 更偏交互式调试、本地文件 patch 和浏览器验证闭环。
...
面板每次发送消息时也会附带 CLI 权限模式。默认是 `auto`；需要只做计划时选择 `plan`，只有在可信的一次性沙盒里才选择 `bypassPermissions`。
...
| 模式 | 发送给 CLI 的参数 |
|---|---|
| 自动 | `--permission-mode auto`（默认） |
| 计划 | `--permission-mode plan` |
| Bypass | `--permission-mode bypassPermissions` |

权限模式下拉框不会开启本地文件写入。无论 Python bridge 还是 Node bridge，`[ACTION:save]` / `write_file` 仍然必须设置 `CC_DEVTOOLS_ENABLE_WRITE=1`。

动作执行默认保持轻量：只读动作自动执行，`click` / `input` / `press` 在 Auto 模式继续自动运行；只有更高风险的 `eval`、`save`、`file:read` 会弹出确认。Plan 模式阻止会改变状态的动作；Bypass 模式跳过面板确认，但不会绕过 bridge 的写入根目录、敏感文件和 `CC_DEVTOOLS_ENABLE_WRITE` 检查。
...
Node bridge 是备选路径，但它使用同样的默认 `auto` 权限模式，也使用同样的 `CC_DEVTOOLS_ENABLE_WRITE=1` 显式写入开关。
...
| `CC_DEVTOOLS_PERMISSION_MODE` | `auto` | 面板未发送权限模式时的默认 CLI 权限模式；可用值包括 `auto`、`plan`、`bypassPermissions` |
...
| `CC_DEVTOOLS_BYPASS` | 未设置 | 兼容旧用法；仅在面板未发送权限模式时作为 `bypassPermissions` 快捷开关 |
...
- 页面 URL、正文、DOM、console、network URL 和 action 结果中匹配常见字段名的 token-like 值会被脱敏。
...
- 面板默认使用 Claude Code `--permission-mode auto`；只有理解风险后才选择 `bypassPermissions`。
- Auto 模式下，`eval`、`save`、`file:read` 需要面板确认；`click`、`input`、`press` 仍然自动执行，方便前端验证闭环。
