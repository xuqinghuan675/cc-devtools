...
Workbench 已实现并有自动化测试覆盖。当前功能形成完整的本地前端调试闭环：

- Workbench 外壳、面板同步护栏、核心数据模型、脱敏 helper、Safety Kernel。
- Evidence Board，支持筛选、选择、复制，以及脱敏后的发送预览。
- Bug Flight Recorder，使用 2 分钟 / 300 事件 / 1 MB 的 ring buffer，并可生成 BugBundle。
- Playwright Test Generator，从选中证据生成只复制、不写文件的测试草稿。
- Patch Transaction，支持预览、备份、应用、人工验证和回滚状态机。
- Visual + DOM Diagnostics，不依赖截图权限即可诊断 DOM、样式、遮挡和可点击性。
- Trust Mode 页面和统一 Send Preview。
- Recipes + Project Memory，支持手动导入导出。

cc-devtools 是成熟的 local-first 开发者 Workbench。更新扩展后仍建议做常规 smoke test；文件写入应继续限制在可信项目根目录。
...
| Visual | DOM 诊断：元素摘要、rect、computed style、disabled、pointer-events、overflow chain、中心点和 `elementFromPoint` 遮挡检查。截图能力以状态显示，当前实现不强依赖截图。 |
...
普通 Chat 发送只会更新 Trust 页面的最近一次 Send Preview，不打断聊天流程。选中证据、BugBundle、测试草稿、补丁/文件内容，以及 Observe Only 下包含 page context 的发送会要求预览确认。action-result 自动回传不会弹预览，以免打断自动验证循环。
...
cc-devtools 是成熟的 local-first 前端调试 Workbench。Python bridge 是主路径；Node bridge 是给偏好 Node.js 的用户保留的备选方案。

Workbench 已实现并有测试覆盖。后续工作集中在更完整的截图能力、iframe/shadow DOM 诊断，以及更多真实 Chrome DevTools 场景的手工 smoke test 覆盖。
